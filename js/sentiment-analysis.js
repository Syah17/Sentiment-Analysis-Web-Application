...
function displayCG(twitterData){
	...
	var chart = new CanvasJS.Chart("chartContainer1",
    {
        ...
        data: [
        {
            ...
            color: "rgba(5,57,153,.98)",
            showInLegend: true,
            legendText: "Tweets total count",
			indexLabel: "{label}  {y}",
            dataPoints: [
                { x: 10, y: (sentimentsCounter["Positive"] % twitterData.length), label: "Positive" },
                { x: 20, y: (sentimentsCounter["Neutral"] % twitterData.length), label: "Neutral" },
                { x: 30, y: (sentimentsCounter["Negative"] % twitterData.length), label: "Negative" },
            ]
        }
        ]
    });
...
function myFunction1() {
  var element = document.body;
  element.classList.toggle("twitter-mode");
}
